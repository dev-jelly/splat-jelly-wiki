...
# DNS Verification Script for sp-wiki.octol.ink
echo "========================================="
echo "DNS Verification Script"
echo "Target: sp-wiki.octol.ink"
echo "========================================="
echo ""
...
if command -v dig &> /dev/null; then
    ...
    DIG_RESULT=$(dig +short sp-wiki.octol.ink)
...
if command -v nslookup &> /dev/null; then
    NSLOOKUP_RESULT=$(nslookup sp-wiki.octol.ink 2>&1 | grep -A 1 "Name:" | tail -1)
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" https://sp-wiki.octol.ink 2>&1 || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    echo "   ✓ Site is accessible (HTTP 200)"
elif [ "$HTTP_CODE" = "301" ] || [ "$HTTP_CODE" = "302" ]; then
    echo "   → Site redirects (HTTP $HTTP_CODE)"
    REDIRECT_URL=$(curl -s -I https://sp-wiki.octol.ink 2>&1 | grep -i "location:" | head -1)
    echo "   $REDIRECT_URL"
...
if command -v openssl &> /dev/null; then
    SSL_INFO=$(echo | openssl s_client -connect sp-wiki.octol.ink:443 -servername sp-wiki.octol.ink 2>&1 | grep "Verify return code" | head -1)
    if [ -n "$SSL_INFO" ]; then
        echo "   $SSL_INFO"
    else
...
echo "   → Run: dig sp-wiki.octol.ink"
